...
\KwIn{Dữ liệu bài toán $data$, hàm dừng $stop\_check$, cờ ghi log $enable\_trace$}
\KwOut{Nghiệm $solution$ và thống kê $stats$}

$nodes\_expanded \leftarrow 0$ \;

\If{$data.n \ge 5$}{
    $enable\_trace \leftarrow False$ \;
}
...
\Else{
    $trace \leftarrow \varnothing$ \;
}

Khởi tạo thời gian $time\_start$ \;
Bắt đầu đo bộ nhớ \;

$domain \leftarrow$ domain\_init($data$) \;
$kb \leftarrow$ build\_kb($data$) \;

$result \leftarrow$ backtrack($kb$, $domain$, $0$, $stop\_check$, $trace$, $enable\_trace$) \;

Ghi nhận $time\_end$, bộ nhớ sử dụng \;

$stats \leftarrow \{runtime, memory, nodes\_expanded, trace\}$ \;

\If{$result = \varnothing$}{
    \Return $(\varnothing, stats)$ \;
}
...
\KwIn{$kb$, $domain$, $depth$, $stop\_check$, $trace$, $enable\_trace$}
...
$nodes\_expanded \leftarrow nodes\_expanded + 1$ \;

$success \leftarrow$ propagate($kb$, $domain$, $depth$, $stop\_check$) \;
...
\For{mỗi $v \in values$}{
    ...
    $result \leftarrow$ backtrack($new\_kb$, $new\_domain$, $depth+1$, $stop\_check$, $trace$, $enable\_trace$) \;
...
\KwIn{$kb$, $domain$, $depth$, $stop\_check$}
...
\While{True}{
    ...
    Thêm các singleton vào $kb$ \;

    \If{$domain = old\_domain$ và không có singleton mới}{
        \textbf{break}
    }
