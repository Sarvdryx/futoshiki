...
\begin{itemize}
    ...
    \item Hàng 3: $\{2,\_\}$ và cột 2 còn $\{3\}$ $\Rightarrow (3,2)=3,\ (3,3)=1$
\end{itemize}
...
\begin{tcolorbox}[colback=gray!5, colframe=black, title=A*solver]
\begin{verbatim}
Function AStarSolver(data, heuristic, is_valid):
    start_time ← current_time()
    initialize memory tracking
    initial_state ← init_state(data)
    PQ ← empty priority queue
    push (f = 0, initial_state) into PQ
    steps ← 0

    While PQ is not empty:
        state ← pop state with smallest f from PQ
        steps ← steps + 1
        If state is goal:
            return solution
        For each child in expand(state, data, is_valid):
            g ← cost(child)              # số ô đã điền
            h ← heuristic(child, data)   # ước lượng còn lại
            f ← g + h
            push (f, child) into PQ
    return failure
