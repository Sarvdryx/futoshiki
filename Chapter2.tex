...
\subsection*{A4 - Không trùng giá trị trong cột}
...
\subsection*{A5 - Bất đẳng thức ngang $<$}
...
\subsection*{A6 - Bất đẳng thức ngang $>$}
...
\subsection*{A7 - Bất đẳng thức dọc $<$}
...
\subsection*{A8 - Bất đẳng thức dọc $>$}
...
\subsection*{A10 - Ràng buộc miền giá trị}
...
Cơ sở tri thức (KB) trong bài toán được biểu diễn dưới dạng \textbf{hội (AND) của các mệnh đề CNF}, trong đó mỗi mệnh đề là một \textbf{tuyển (OR)} của các literal.

KB gồm hai thành phần chính:

\begin{itemize}
    \item \textbf{Facts (Sự thật):} Là các thông tin đúng ngay từ đầu, bao gồm:
    \begin{itemize}
        \item Các ô đã cho sẵn giá trị: \texttt{Given(i,j,v)}
        \item Các ràng buộc bất đẳng thức: \texttt{LessH}, \texttt{GreaterH}, \texttt{LessV}, \texttt{GreaterV}
    \end{itemize}
    
    \item \textbf{Rules (Luật):} Được chuyển về dạng CNF để phục vụ suy diễn logic.
\end{itemize}

Từ dữ liệu đầu vào của bảng $N \times N$, hệ thống tự động sinh KB dựa trên các tiên đề sau:

\begin{itemize}

    \item \textbf{A1 -- Mỗi ô có ít nhất một giá trị:}
    \[
    \bigvee_{v=1}^{N} Val(i,j,v)
    \]
    Đảm bảo mỗi ô $(i,j)$ phải nhận một giá trị trong miền.

    \item \textbf{A2 -- Mỗi ô có nhiều nhất một giá trị:}
    \[
    \neg Val(i,j,v_1) \lor \neg Val(i,j,v_2), \quad v_1 \neq v_2
    \]
    Đảm bảo một ô không thể nhận hai giá trị khác nhau.

    \item \textbf{A3 -- Không trùng giá trị trên cùng hàng:}
    \[
    \neg Val(i,j_1,v) \lor \neg Val(i,j_2,v), \quad j_1 \neq j_2
    \]

    \item \textbf{A4 -- Không trùng giá trị trên cùng cột:}
    \[
    \neg Val(i_1,j,v) \lor \neg Val(i_2,j,v), \quad i_1 \neq i_2
    \]

    \item \textbf{A5 -- Ràng buộc nhỏ hơn ngang:}
    Với ràng buộc $(i,j) < (i,j+1)$:
    \[
    \neg LessH(i,j) \lor \neg Val(i,j,v_1) \lor \neg Val(i,j+1,v_2), \quad v_1 \ge v_2
    \]

    \item \textbf{A6 -- Ràng buộc lớn hơn ngang:}
    Với ràng buộc $(i,j) > (i,j+1)$:
    \[
    \neg GreaterH(i,j) \lor \neg Val(i,j,v_1) \lor \neg Val(i,j+1,v_2), \quad v_1 \le v_2
    \]

    \item \textbf{A7 -- Ràng buộc nhỏ hơn dọc:}
    Với ràng buộc $(i,j) < (i+1,j)$:
    \[
    \neg LessV(i,j) \lor \neg Val(i,j,v_1) \lor \neg Val(i+1,j,v_2), \quad v_1 \ge v_2
    \]

    \item \textbf{A8 -- Ràng buộc lớn hơn dọc:}
    Với ràng buộc $(i,j) > (i+1,j)$:
    \[
    \neg GreaterV(i,j) \lor \neg Val(i,j,v_1) \lor \neg Val(i+1,j,v_2), \quad v_1 \le v_2
    \]

    \item \textbf{A9 -- Luật gán giá trị từ dữ kiện:}
    \[
    Given(i,j,v) \Rightarrow Val(i,j,v)
    \]
    Được chuyển sang CNF:
    \[
    \neg Given(i,j,v) \lor Val(i,j,v)
    \]

\end{itemize}

Cuối cùng, toàn bộ KB được xây dựng bằng cách lấy hội của tất cả các mệnh đề trên và những facts ban đầu:
\[
KB = A1 \wedge A2 \wedge A3 \wedge A4 \wedge A5 \wedge A6 \wedge A7 \wedge A8 \wedge A9 \wedge ...
\]

Quá trình này được thực hiện tự động từ dữ liệu đầu vào, bao gồm việc duyệt qua bảng, trích xuất các dữ kiện ban đầu và sinh các ràng buộc tương ứng.
